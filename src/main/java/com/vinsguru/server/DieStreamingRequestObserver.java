...
public class DieStreamingRequestObserver implements StreamObserver<Die>
{
    private Player client;
    private Player  server;
    private StreamObserver<GameState> gameStateStreamObserver;

    public DieStreamingRequestObserver(Player client, Player server, StreamObserver<GameState> gameStateStreamObserver) {
        this.client = client;
        this.server = server;
    ...
    private GameState getGameState(){
        return GameState.newBuilder().addPlayer(this.client)
    ...
    private Player getNewPlayerPosition(Player player,int dieValue){
        ...
        position = SnakeAndLadderMap.getPosisition(position);
        if(position<=100){
